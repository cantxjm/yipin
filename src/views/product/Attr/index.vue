    ...
    <el-card>
      ...
      <div v-show="!isShowTable">
        ...
        <el-button
          ...
          >添加属性值</el-button
        >
        <el-button @click="isShowTable = true" type="info">取消</el-button>
        <el-table
  ...
  methods: {
    ...
    addAttrValue() {
      ...
      this.attrInfo.attrValueList.push({
        ...
        valueName: "",
        flag: true, //给每一个属性值都添加了flag，更好的切换查看与编辑模块（flag是一个响应式数据）
      });
      this.$nextTick(() => {
    ...
    // 失去焦点的事件---展示span
    toLook(row, index) {
      // 如果属性值为空则不能作为新的属性值，需要给用户提示，让他输入一个其他的属性值
    ...
    async addOrUpdateAttr() {
      ...
      this.attrInfo.attrValueList = this.attrInfo.attrValueList.filter(
        (item) => {
          ...
          if (item.valueName.trim() != "") {
            ...
            // 删除掉flag为true的item（相当于把属性值为空的对象删除）
            item.flag !== true;
          }
        }
      );
      try {
        if (
          ...
          // 发请求
          await this.$API.attr.reqAppOrUpdateAttr(this.attrInfo);
          // 展示平台属性的信号量进行切换
